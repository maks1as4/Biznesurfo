...
if (isset($_GET['tid']) && !empty($_GET['tid']) && CheckIntegerUnsign($_GET['tid'])){
	...
	if (isset($user_id) && CheckIntegerUnsign($user_id)){
		...
		header('Location: /enter?link=/archive/product-delete/'.$tovar_id);
	
}else
	require_once 'page404.php';

$active_tab = 4;
$title = 'Ћичный кабинет - јрхив - удаление товара';

require_once '../../includs/control/archive_product_delete.html';
