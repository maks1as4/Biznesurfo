...
if (isset($user_id) && CheckIntegerUnsign($user_id)){
	...
	if (isset($_SESSION['add_warning']) && !empty($_SESSION['add_warning']) && $_SESSION['add_warning'] == 'show'){
		$show_add_warning = true;
		unset($_SESSION['add_warning']);
	}
...
#---------------------- Фильтры ----------------------
	...
	$qRes = SqlQuery("Select distinct(S.`rubric`), R.`name` From `STR` S join `RUBRICS` R on S.`rubric`=R.`id` Where S.`client`='".mysql_real_escape_string($user_client_id)."' and S.`active`='1' Order by R.`id`;");
	...
	if (isset($_SESSION['rfilter']) && !empty($_SESSION['rfilter'])){
		$filter .= 'S.`rubric` in (';
		foreach ($_SESSION['rfilter'] as $f)
			$filter .= $f.',';
	...
	if (isset($_SESSION['count_products']) && !empty($_SESSION['count_products'])){
		$conut_pages = $_SESSION['count_products'];
	}
		...
		if (isset($_POST['action']) && !empty($_POST['action'])){
			if ($_POST['action'] == 'filter_rubrics'){
				...
				if (isset($_POST['rub_filter']) && !empty($_POST['rub_filter'])){
					$_SESSION['rfilter'] = $_POST['rub_filter'];
					$filter = '';
					$filter .= 'S.`rubric` in (';
					...
					if (isset($_SESSION['rfilter'])) unset($_SESSION['rfilter']);
					$filter = '';
				}
				...
				if (isset($_POST['count']) && !empty($_POST['count'])){
					...
					if (in_array($_POST['count'], $counts_validate) && $_POST['count'] != 20){
						$_SESSION['count_products'] = $_POST['count'];
						$conut_pages = $_POST['count'];
					}else{
						if (isset($_SESSION['count_products'])) unset($_SESSION['count_products']);
						$conut_pages = 20;
					}
				}else{
					if (isset($_SESSION['count_products'])) unset($_SESSION['count_products']);
...
#---------------------- Сортировка товаров ----------------------
	...
	// иницилизация параметров ссылок
	$page_url = 'http://'.$baseHref.'/kabinet';
	...
	// инициализация параметров сортировки
	$order = 'R.`name`'; // сортировка в SQL
	$by = 'asc'; // порядок сортировки
...
#---------------------- Постраничная разбивка ----------------------
	...
	$pagNam = '/kabinet'; // Сборка URL для пейджинга
	$qtyPerPage = $conut_pages; // Число строк на странице
	$countPages = 10; // Число страниц в ленте
	
	$qRes = SqlQuery("Select count(*) from `STR` S Where ".$filter."S.`client`='".mysql_real_escape_string($user_client_id)."' and S.`active`='1';");
	if (mysql_num_rows($qRes) === 1){
	...
	$qRes = SqlQuery("
		...
		Where ".$filter."S.`client`='".mysql_real_escape_string($user_client_id)."' and S.`active`='1'
...
$(document).ready(function() {
	$('a.delete-product').bind('click', function(e) {
		...
		delete_modal('Вы уверенны, что хотите удалить товар?', '30%', function() {
			window.location = '/kabinet/product-delete/' + id + '?confirm';
		});
		return false;
	});
	...
	$('#show-all').tooltip({
		title: 'Возможно долгая загрузка страницы.',
		trigger: 'hover'
	});
	...
	$('#multi-delete').click(function() {
		if (!$(this).hasClass('disabled')) {
			delete_modal('Удалить выбранные товары?', '30%', function() {
...
$rubric_filtre_type = 'rfilter';
$active_tab = 0;
$title = 'Личный кабинет - товары компании';

require_once '../../includs/control/index.html';
