...
if (isset($_GET['nid']) && !empty($_GET['nid']) && CheckIntegerUnsign($_GET['nid'])){
	...
	if (isset($user_id) && CheckIntegerUnsign($user_id)){
		...
		$val_title = $News[0];
		$val_text  = $News[1];
		$val_img_src = (($News[2] == '') || ($News[3] == '')) ? '/i/kabinet/no-image.png' : '/i/news/'.$News[2].'_small.'.$News[3]; // картинка
								...
								if ($pictures->uploaded){
									...
									if ($pictures->processed){
										...
										if ($pictures->processed){
											...
											if (($News[2] != '') && ($News[3] != '')){ // наличие картинки
...
$active_tab = 2;
$title = 'Личный кабинет - изменить новость';
