...
if (isset($user_id) && CheckIntegerUnsign($user_id)){
	...
	if (count($_POST)>0){
		...
		if ($err == ''){
			...
			SqlQuery("Insert Into `STR` Set `client`='".mysql_real_escape_string($user_client_id)."', `rubric`='".mysql_real_escape_string($val_rub_id)."', `name`='".mysql_real_escape_string($val_name)."', `translit`='', `dop`='".mysql_real_escape_string($dop_db)."', `price`='".mysql_real_escape_string($price_db)."', `dog`='".mysql_real_escape_string($dog_db)."', `text`='".mysql_real_escape_string($val_text)."', `active`='1', `status`='0', `adate`='".$now."', `udate`='".$now."';");
			...
			if ($save_pic)
				SqlQuery("Insert Into `STR_IMG` Set `id_product`='".$id_last."', `name`='".$random_file_name."', `ext`='".$ext."', `sort_order`='1';");
			// логируем добавленный товар
			$log_new = 'rubric - '.$val_rub_id.'; name - '.$val_name.'; price - '.$price_db.'; text - '.$val_text;
			SqlQuery("Insert Into `LOG` SET `id_member`='".mysql_real_escape_string($user_id)."', `type`='2', `action`='0', `old`='-', `new`='".mysql_real_escape_string($log_new)."', `adate`='".$now."';");
	...
	header('Location: /enter?link=/product-add');
...
require_once '../../includs/control/product_add.html';
