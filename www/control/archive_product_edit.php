...
if (isset($_GET['tid']) && !empty($_GET['tid']) && CheckIntegerUnsign($_GET['tid'])){
	...
	if (isset($user_id) && CheckIntegerUnsign($user_id)){
		...
		$qRes = SqlQuery("
			...
			Where S.`id`='".mysql_real_escape_string($tovar_id)."' and S.`client`='".mysql_real_escape_string($user_client_id)."' and S.`active`='0';
		");
		...
		if (count($_POST)>0){
			...
			if (isset($_POST['text']) && ($_POST['text']!='')){
				...
				if ($str_length <= 10000){
					...
					$config = HTMLPurifier_Config::createDefault();
					$config->set('HTML.Allowed', 'p,strong,b,em,i,ul,ol,li,h3,hr'); // допустимые теги (a[href])
				...
				if (in_array($info['mime'], $allowed_mime)){ // проверка типа файла
					if ($_FILES['upicture']['size'] != 0){ // проверка на пустой файл
						if ($_FILES['upicture']['size'] <= 1024*1024*2){ // 2Мб допустимый размер загружаемой картинки
							...
							$err .= ' - Загружаемый логотип превысил лимит по размеру, допустимы файлы не более 1Мб.<br />';
					}else
		...
		header('Location: /enter?link=/archive/product-edit/'.$tovar_id);
...
$(document).ready(function() {
	...
	$('a.delete-product-image').click(function(e) {
		...
		delete_modal('Вы уверенны, что хотите удалить картинку?', '30%', function() {
			$.ajax({
				type: 'POST',
				url: '/ajax/archive_product_delete_image.php',
				data: {'id':id},
				dataType: 'json',
...
$active_tab = 4;
$title = 'Личный кабинет - Архив - обновление товара';

require_once '../../includs/control/archive_product_edit.html';
