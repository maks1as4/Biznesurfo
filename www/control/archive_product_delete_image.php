...
if (isset($_GET['pid']) && !empty($_GET['pid']) && CheckIntegerUnsign($_GET['pid'])){
	...
	if (isset($user_id) && CheckIntegerUnsign($user_id)){
		...
		if (isset($_GET['confirm']) && empty($_GET['confirm'])){
			...
			// переход на страницу управлени€ товарами
			header('Location: /kabinet/archive/product-edit/'.$tovar_id);
		}
		
	}else
		header('Location: /enter?link=/archive/product-delete-image/'.$image_id);
	
}else
	require_once 'page404.php';

$active_tab = 4;
$title = 'Ћичный кабинет - јрхив - удаление изображени€ товара';

require_once '../../includs/control/archive_product_delete_image.html';
