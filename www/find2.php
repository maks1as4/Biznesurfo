...
if ($cnt_wrds>0) {
	...
	$qTxt = 'select '.
			...
			'R.id_parent,'.		// 14
			'R.new_url,'.		// 15
			'GROUP_CONCAT(SI.pos order by 1) '.	// 16	
			'from WORDFORMS W '.
			'JOIN STR_INDEX SI on W.base=SI.word '.
			'JOIN STR S on SI.str=S.id '.
			...
			for ($i=0; $i<$cnt_wrds; $i++){
				...
				$qTxt .= (($i>0)?',':'').'"'.strtoupper(addslashes($wrds[$i])).'"';
			}
			$qTxt .= ') and S.active=1 and S.status=0 ';
			if ($idReg>0) {
				if ($isReg)
	...
	if (($qRes)&&mysql_num_rows($qRes)>0) {
		...
		$relCol = count($Ress[0])-1;
		if ($doRelav) {
			for ($i = 0; $i<$RowsQty; $i++)
				$Ress[$i][$relCol] = CalcRelav($Ress[$i], strtoupper($fText));
		}
		qSort($Idx, $Ress);
		...
		for ($j = $k; $j<$i+$k; $j++) {
			...
			$Lins[$n][1] = DoStrongWords($Ress[$Idx[$j]][1]).'&nbsp;&nbsp;&nbsp;('.$Ress[$Idx[$j]][$relCol].') '.$Ress[$Idx[$j]][16];
			$Lins[$n][2] = $prc; // цена
...
//		$HeadersQty = BuildHeaders($s);
		...
		if (count($arrRubrics)>0) {
			$RightTextMods = GetRightTextBlocksFromArray(4);
			$RightBanners = GetRightBannersFromArray(4);
...
function CalcRelav($mName, $fndT) {
	...
	if (isset($fndT)&&($fndT!='')) {
		...
		return 0;
	$cnt_wrds = count($wrds);
	$arrPos = explode(',',$mName[16]);
	
	$valRel = 0;
	for($i=0;$i<count($arrPos);$i++){
		if($i>0)
			$valRel += ($arrPos[$i]-$arrPos[$i-1]);
	}
	$valRel = (1000-$valRel)+(count($arrPos)*1000)-($arrPos[0]*100);
	
	return $valRel;
	
/*	$n = preg_match_all('([А-ЯЁа-яёA-Za-z\d-\+\\{}\(\)\"]+((?<=\d)[,.](?=\d)\d+)*)', $mName[2], $arrs);
