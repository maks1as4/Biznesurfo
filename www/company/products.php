...
// сортировка
$product_page = 'http://'.$translit_company.'.'.$url3Href.'/products';
$link_rubric = $product_page.'?order=rubric&by=desc&p='.$curPageNo;
$arr_rubric = 'asc';
...
if (isset($_GET['order']) && !empty($_GET['order'])){
	...
	switch ($_GET['order']){
		case 'rubric':{
			$link_rubric = $product_page.'?order=rubric&by='.$not_by.'&p='.$curPageNo;
			$arr_rubric = $by;
			$link_name = $product_page.'?order=name&by=asc&p=1';
			$arr_name = '';
			$link_price = $product_page.'?order=price&by=asc&p=1';
		...
		case 'name':{
			$link_rubric = $product_page.'?order=rubric&by=asc&p=1';
			$arr_rubric = '';
...
$pagNam = 'http://'.$translit_company.'.'.$url3Href.'/products';
...
$qRes = SqlQuery("Select count(*) From `STR` S Where S.`client`='".mysql_real_escape_string($id_company)."' and S.`active` = '1'".$sql_filter.";");
...
$Products = array();
$qRes = SqlQuery("
	Select S.`rubric`, R.`name`, S.`name`, S.`translit`, S.`dop`, S.`price`, S.`dog`,
	...
	Where S.`client` = '".mysql_real_escape_string($id_company)."' and S.`active` = '1'".$sql_filter."
...
$order_box = true;
$action = 'catalog';
$title = 'Прайс-лист компании '.$name.' - '.$sity_name.', каталог, прайс, цены, сравнение цен '.$name;
$descr = 'Информация о товарах компании '.$name.' - '.$sity_name.', просмотр цен, возможность заказать товар или услугу компании '.$name;
$keyws = $name.', прайс-лист, цены, каталог, сравнение, прайс';

$template = 'default/products.html';
