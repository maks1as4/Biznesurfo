...
$whr = ' Where C.page = 2 and C.rubric > 0 and '.$range;
...
if ((mysql_num_rows ($qRes) > 0) && ($qRes)) {
	while ($Rows = mysql_fetch_row ($qRes)) {
		...
		$Rubs[$i][2] = $Rows[2];
		$Rubs[$i][3] = 0;
		$qRes2 = mysql_query ('Select SUM(C.visits) from COUNTERS C Where C.page = 2 and C.rubric = '.$Rows[2].' and '.$range.';');
...
$qRes = mysql_query ('Select R.name, R.id From RUBRICS R Where exists (Select * From STR S where S.rubric=R.id) and not exists (Select * From COUNTERS C where C.page = 2 and C.rubric = R.id and '.$range.') Order by R.name;');
if ((mysql_num_rows ($qRes) > 0) && ($qRes)) {
	while ($Rows = mysql_fetch_row ($qRes)) {
		$Rubs[$i][0] = $Rows[0];
		$Rubs[$i][1] = 0;
